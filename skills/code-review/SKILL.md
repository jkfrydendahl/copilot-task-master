...
After running a multi-model code review using the models configured in the shared `config/review-models.md` (in your master instructions folder), use this skill to synthesize the independent findings into a single prioritized report.

> **The review criteria and severity levels are defined in the Code Review Standards instruction (loaded with your instructions)** — they are applied automatically during each `/review` pass. This skill handles only the **synthesis** of results across models.
...
Run this skill after completing a multi-model review. Use the models and `/review` command from the shared `config/review-models.md`.
...
See the shared `config/review-models.md` for the current model list and review command.
...
**Models Used**: [List models from the shared config/review-models.md]
